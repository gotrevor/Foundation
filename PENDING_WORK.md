...
- ⬜ meta `bv_fixitr_fvSup : bv(fixitr 0 (fvSup χ) ▹ χ) = fvSup χ` AND its internal-`bv` quote form
  `bv ℒₒᵣ ⌜fixitr ▹ χ⌝ = fvSup χ` — pins `m`. (Needs: `&(fvSup-1)` occurs in χ — "sup attained".)
- ⬜ extract `indBodyGraph`/Σ₁-graph of `indBody.val`; internal Δ₁ assembly of `ch` via
  ...
  → `ProperOn` is FREE (`Defined.proper`). (Infra validated by `qqAllsDef`/`fvarVecDef` patterns.)
- ⬜ `mem_iff` over ℕ. **Forward** (member ⟹ recognizer): all verified bridges now in place —
  `quote_univCl_eq` (p = qqAlls b m, m=0+fvSup), `subst_fvarVec_quote` (subst recovers ⌜succInd ψ⌝),
  `indBody_quote` (= indBody ⌜ψ⌝). **Backward**: inversion bijection {FV-free,bv=m}≅{fvSup=m} +
  `IsSemiformula.sound` to get ψ from K.
