...
lemma Sequent.weight_lt_weight_of_mem_reduction {h : ¬Γ.IsAtomic} : Δ ∈ Γ.reduction h → Δ.weight < Γ.weight := by
  ...
  |        ⊥ =>
    ...
    have : ⊥ ∈ Γ := by have hm := Sequent.chooseNonAtomic_mem h; rw [H] at hm; exact hm
  ...
  |    φ ⋏ ψ =>
    have : φ ⋏ ψ ∈ Γ := by have hm := Sequent.chooseNonAtomic_mem h; rw [H] at hm; exact hm
    suffices
...
def Derivation.ofReduction {Γ : Sequent α} {hΓ : ¬Γ.IsAtomic}
    ...
    |        ⊤ => verum' (by have hm := Sequent.chooseNonAtomic_mem hΓ; rw [H] at hm; exact hm)
    |        ⊥ =>
    ...
    |    φ ⋏ ψ =>
      ...
      dφ.and dψ |>.wk (by
        have hm : φ ⋏ ψ ∈ Γ := by
          have h2 := Sequent.chooseNonAtomic_mem hΓ; rw [H] at h2; exact h2
        simp [hm])
    |    φ ⋎ ψ =>
