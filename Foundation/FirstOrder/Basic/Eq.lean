...
lemma eval_mk {e} {ε} {φ : Semiformula L μ n} :
  ...
  case hall n φ ih =>
    constructor
    · intro h a; exact (ih (e := a :> e)).mp (by simp only [Matrix.comp_vecCons]; exact h ⟦a⟧)
    · intro h a;
  ...
  case hexs n φ ih =>
    ...
    · intro ⟨a, h⟩
      induction' a using Quotient.ind with a
      exact ⟨a, (ih (e := a :> e)).mp (by simp only [Matrix.comp_vecCons]; exact h)⟩
...
lemma models_iff {σ : Sentence L} : QuotEq L M ⊧ₘ σ ↔ M ⊧ₘ σ := by
  have h := eval_mk₀ (M := M) (φ := σ) (ε := Empty.elim)
  simp only [Empty.eq_elim] at h; exact h
